...
**Date:** 2026-09-22
**Change:** Removed the "Send Status Update" email feature entirely, rather than configuring it. Deleted `server/src/lib/mail.js` and `statusEmail.js`, the two routes that used them (`GET /status-email-ready`, `POST /send-status-email`) in `dashboard.js`, the `StatusEmailButton` component and both its render sites in `Dashboard.jsx`, the now-unreachable `status_email`/`sent` audit-log rendering branch in `AuditTimeline.jsx`, the `SMTP_*`/`STAKEHOLDER_EMAILS` block in `.env.example`, and the `nodemailer` dependency. `CLAUDE.md` §16 ("Status Email") body replaced with a removal note — section number kept, not reused.
**Reason:** Built 2026-09-15, the feature was code-complete but never configured — it needed real SMTP credentials and a stakeholder email list that were never supplied. Asked to set those up, the answer was to remove the feature instead of maintaining it unconfigured.
**Files:** `server/src/lib/mail.js` (deleted), `server/src/lib/statusEmail.js` (deleted), `server/src/routes/dashboard.js`, `client/src/pages/Dashboard.jsx`, `client/src/components/AuditTimeline.jsx`, `.env.example`, `CLAUDE.md`, `server/package.json`.
**Database:** No schema change. No audit_logs rows ever existed for this feature — it was never configured, so `sendMail` always failed before `recordAudit` was reached — so nothing needed preserving, unlike the Blocker/Achievement removal where real historical data remained.
**Tests:** No existing tests referenced this feature (it was never exercised end-to-end). `npm run build` clean (client). Full backend suite re-run to confirm zero regressions from the removal.
**Deployment:** Live.
**Cost:** None — one fewer dependency (`nodemailer`), no ongoing SMTP account to maintain.

---

**Date:** 2026-09-22
**Change:** Full end-to-end functional audit of the entire application (every role, every screen), followed by fixes for everything the audit confirmed as real. Six changes, all already logged individually below at the file level — this entry is the audit summary that ties them together. (1) `users.js`/`Admin.jsx`: deactivating or demoting a manager who still has active reports is now blocked (matched DELETE's existing guard onto PATCH, and blocked assigning a deactivated person as anyone's manager). (2) `requests.js`/`AuditTimeline.jsx`: support-request approve and reject now write distinct audit entries instead of an identical one. (3) `dashboard.js`/`Dashboard.jsx`/`scrum.js`/`teams.js`/`InfoTip.jsx`: removed the half-built Escalations feature (zero live callers), fixed "Repeated Support Requests" to read real request history instead of current status, rebuilt the Org Dashboard's "By Team" table onto the same computed-leader logic the rest of the app already uses, and added an org-wide "Needs Attention" panel to the Org Dashboard. (4) `history.js`/`History.jsx`: added an Overdue filter, a Completed At column, and a truncation notice for large result sets. (5) `TeamToday.jsx`: added a note clarifying that Team Overview's Day view only fully reflects a past date's real activity via History (Today's Work/Delayed/Support columns are always live, not date-scoped), and made Month grid day cells link into History. (6) Admin's manager picker now excludes deactivated users.
**Reason:** User-directed full-application audit brief: understand the app as a real business product (not a bug hunt), use it from every role's perspective, produce a complete gap register, then actually implement and re-test the fixes — not just report them.
**Scope of the audit itself:** 5 parallel research passes covering hierarchy/accountability, the Scrum Yesterday/Today/Support workflow, calendar/history, leader action management, and management visibility (Org/Senior Management dashboards) — each explicitly briefed on this app's real, current data model (no Blocker/Achievement/Department/Sprint concepts; those were deliberately removed earlier, see the 2026-09-09 entries) so findings weren't measured against a fictional target. Findings were value-tested (business value, frequency, accountability impact, complexity) before anything was implemented — several plausible ideas were deliberately NOT built: a formal Open→Assigned→In Progress→Completed→Verified task-status lifecycle (the current model already covers this via status + due_date + support flag, and adding stages here was judged as bureaucracy without a matching accountability gain), restoring a "Delayed" KPI tile to the dashboards (the existing Overdue filter in History covers the same need without adding a fourth permanently-visible number to screens this app's own prior audit had deliberately simplified), and a "this week vs last week" trend stat (no clear owner action would follow from it yet).
**Files:** `server/src/routes/users.js`, `server/src/routes/requests.js`, `server/src/routes/dashboard.js`, `server/src/routes/scrum.js`, `server/src/routes/teams.js`, `server/src/routes/history.js`, `client/src/pages/Admin.jsx`, `client/src/components/AuditTimeline.jsx`, `client/src/components/InfoTip.jsx`, `client/src/pages/Dashboard.jsx`, `client/src/pages/History.jsx`, `client/src/pages/TeamToday.jsx`.
**Database:** No schema change. The Escalations removal did not touch the underlying table — only the two dead route handlers, the dead dashboard query, and the dead glossary entry.
**Tests:** 5 new targeted tests added (deactivated-manager rejected on create and on edit, a manager-with-active-reports blocked on PATCH, History's Overdue filter + truncation flag, the Org Dashboard's rebuilt By Team shape, and confirmation the removed Escalations routes now 404). Full suite run before adding any new tests (to isolate regressions from the fixes themselves): 67/67 passing, zero regressions. Full suite after adding new tests: 72/72 passing. `npm run build` clean.
**Deployment:** Live.
**Cost:** None.
...
**Date:** 2026-09-22
**Change:** Recurring tasks now generate their next occurrence on schedule, not only when someone completes the current one. Previously a series nobody ever completed just stalled forever — there was no cron or scheduler anywhere in the app. Added a second, schedule-triggered path (`generateDueOccurrences()`) that runs once a day via a new Vercel Cron job and catches up any series that fell behind its own frequency rule, materializing exactly the one current occurrence it's now due for (not one row per missed day — a long-dormant Daily series gets one correctly-overdue task back, not a pile of backlog). The old, never-completed occurrence is left untouched, still visible and trackable as its own delayed item, same as before.
**Reason:** User-reported expectation that recurring tasks "automatically trigger based on the frequency," explicitly compared to how a Teams/Outlook meeting series works (every occurrence exists on its own schedule, independent of whether the previous one was ever dealt with) — confirmed via a screenshot of Teams' own recurrence UI. Investigation confirmed the gap: completion was genuinely the *only* trigger that had ever existed.
**Files:** `server/src/lib/recurringOccurrences.js` (new — `insertOccurrence()`, factored out of scrum.js's existing completion-triggered handler as a pure extraction so both triggers share one insert path instead of two that could drift; `generateDueOccurrences()`, the new sweep), `server/src/routes/cron.js` (new — `GET /api/cron/generate-recurring`, authenticated by a `CRON_SECRET` shared secret since a cron request carries no user session), `server/src/routes/scrum.js` (completion handler refactored to call the shared `insertOccurrence()`), `server/src/index.js` (mounts the new route), `vercel.json` (new `crons` entry, once daily at 03:00 UTC), `.env.example` (documents `CRON_SECRET`, same pattern as `JWT_SECRET` — the user generates and sets it themselves; the route fails closed without it).
**Database:** New partial unique index, `idx_commitments_recurring_due_unique` on `commitments(recurring_activity_id, due_date) WHERE recurring_activity_id IS NOT NULL AND is_active = 1`, applied directly to the live database. Added in direct response to a real bug an adversarial review caught before this shipped (see Security below) — not a hypothetical hardening.
**Bugs found and fixed before shipping, via a 4-lens adversarial review (date-math, idempotency, security, consistency) run against the new code before committing:** (1) Two overlapping triggers (a retried/double-fired cron invocation, or the sweep racing an employee's completion at the same moment) could both pass their own "does this already exist?" check and both insert a duplicate occurrence — the check and the insert aren't atomic across two connections. Fixed with the new unique index above plus graceful conflict handling: the loser of the race returns the winner's row instead of erroring or duplicating. (2) A long-dormant, occurrence-count-limited series (`end.type='after_count'`) could overshoot its configured limit — the internal walk-forward loop checked the end condition against a frozen `occurrences_created` snapshot instead of simulating it advancing hop by hop, so a series allowed 3 occurrences and dormant 12 days would land on today's date instead of stopping at its true 3rd/final occurrence. Fixed by simulating the count at each step of the walk. (3) The cron endpoint's secret check used a plain `!==` comparison (a timing side-channel) — switched to `crypto.timingSafeEqual`. (4) `insertOccurrence()` sourced the new commitment's `employee_id` from the wrong parameter — behaviorally identical today (nothing lets a commitment's owner diverge from its series' owner yet) but a latent footgun for any future reassignment feature — corrected to source every field from the same `template` object consistently.
**Known, pre-existing characteristic, not introduced by this change:** a Monthly series that crosses a short month (e.g. due the 31st, then hits February) permanently drifts its effective day-of-month downward and never recovers — this is how `recurrence.js`'s existing month-clamping already behaved for the completion-triggered path; this change just means it can now also happen automatically for a series nobody is actively completing. Flagged, not fixed — a genuine design question (matches how Outlook itself handles a monthly-on-the-31st series), not a bug from this work.
**Tests:** 7 new tests — a stalled Daily series advancing without completion, idempotency on repeat runs, a series already current being left alone, a series past its end condition being deactivated, the long-dormant count-limited overshoot scenario (proving fix #2 above), a real concurrent-insert race producing exactly one row (proving fix #1 above), and the cron route's auth (no secret / wrong secret / a valid user login / the correct secret). Full suite: 79/79 passing, zero regressions. `npm run build` clean.
**Deployment:** Live — but inert until the user sets a real `CRON_SECRET` on Vercel (Project Settings → Environment Variables) and redeploys; until then the route fails closed (401 for everyone) and recurring series behave exactly as before this change, completion-triggered only.
**Cost:** None — Vercel Cron's free tier covers one daily invocation easily.

---

**Date:** 2026-09-22
**Change:** A full-application audit pass (business process, permissions, data integrity, security, negative testing, UX consistency, reporting reconciliation, production readiness), plus data cleanup and two real bugs found and fixed. Data: deleted two placeholder/test tasks ("sadsada", "zxczxcz") that had been sitting in production History/Team Tasks/Recurring Tasks, paused the underlying stale "sadsada" recurring series (6 days past its own configured end date), fixed the "Achivements" Task Type typo to "Achievements", and linked the "FP&A" Main Task to the "Finance" Subtask (matching the app's own help text, which already used that exact pairing as its example). Code: (1) fixed a race condition in Requests approve/reject where two concurrent resolutions of the same request could both apply their side effects before either's status write landed — now claimed atomically as the first write, with the loser refused (409) before any side effect runs; (2) fixed the personal dashboard's own summary counts (pending/in_progress/support_required/totalDue/totalCompleted) lacking the `is_active=1` filter its own detail lists — and every equivalent count on the Leader/Org dashboards — already had; currently harmless (no code path ever deactivates a commitment) but a latent inconsistency risk.
**Reason:** User-directed full-application audit and production-readiness pass. An initial attempt at an 8-agent parallel audit workflow hit a session usage limit before producing results; the audit continued as a direct, targeted investigation instead — systematically checking every route's ownership guard, grepping for SQL-injection patterns, and re-verifying the live app.
**Files:** `server/src/routes/requests.js`, `server/src/routes/dashboard.js`, `server/test/api.integration.test.js`. Data-only changes made live via Admin (no code): task deletion, recurring-series pause, Task Type rename, Main Task/Subtask link.
**Database:** No schema change. No SQL-injection risk found — every route consistently uses `?`-bound placeholders; every interpolated fragment found is pre-built WHERE-clause text, never raw request values spliced into SQL.
**Also checked and confirmed correct, not changed:** every mutating `:id` route in `scrum.js` has its ownership check (`assertCanEdit`) immediately after loading the record, including the bulk CSV import path; `leader.js`'s `GET /org-tasks` already precomputes its `can_act` set before mapping (an old, already-resolved concern from an earlier design note, not a live bug); `requireAuth` re-checks `is_active=1` on every single request, so a deactivated user's still-valid JWT stops working on their very next request, not just at natural token expiry.
**Tests:** 2 new tests for the requests race fix (one exercises the atomic conditional UPDATE directly at the SQL level, since an HTTP-level `Promise.all` race isn't reliably tight enough to prove it; one confirms a second approve/reject attempt on an already-resolved request is refused end-to-end). Full suite: 81/81 passing. `npm run build` clean.
**Deployment:** Live.
**Cost:** None.
...
**Date:** 2026-09-22
**Change:** Added Activities — a new master-data level below Main Task (Main Task "FP&A" → Activity "Rolling forecast updates"). Picking one on any task-creation form auto-fills the description/title (still editable); switching to a different Activity updates an untouched auto-fill without ever overwriting a manual edit. New Admin "Activities" tab (full CRUD + CSV import, mirroring Main Tasks' own pattern exactly). Cascading Subtask → Main Task → Activity field added to the shared Create Task form (My Tasks / Team Tasks) and Admin's "Build a Recurring Task" form; an independent Activity filter added to Team Tasks' and History's filter bars, plus a new Activity column in History's table. Populated with the real Finance activity catalog the client provided: 12 Main Tasks (Finance Heads — FP&A, General Ledger & Accounting, Accounts Receivable (O2C), Accounts Payable (P2P), Treasury & Cash Management, Budgeting & Cost Control, Taxation & Compliance, Payroll & Employee Finance, Internal Control & Audit, Business Partnering, Systems & Process Excellence, Stakeholder & Management Reporting) and 68 Activities under them, all linked to the existing Finance Subtask.
**Reason:** Client-directed request, working from a real internal Finance activity-catalog spreadsheet — wanted the app's existing Subtask → Main Task structure extended one level deeper so routine, catalog-able work doesn't need its description retyped by hand each time.
**Files:** `server/src/routes/taskActivities.js` (new), `server/migrations/001_schema.sql` (new `task_activities` table + `task_activity_id` FK on `commitments`/`recurring_activities` + indexes), `server/src/index.js`, `server/src/routes/scrum.js`, `server/src/routes/recurringTasks.js`, `server/src/routes/leader.js`, `server/src/routes/history.js`, `server/src/lib/recurringOccurrences.js`, `client/src/pages/Admin.jsx`, `client/src/components/TeamTaskList.jsx`, `client/src/pages/History.jsx`, `client/src/components/AuditTimeline.jsx`.
**Naming note:** the new table is `task_activities`, not `activities` — this codebase already uses "activity"/"activities" heavily for the unrelated `recurring_activities` table (a recurring task's own template row) and `escalations.activity_id`; a bare `activities` table would have collided with both in code, audit `table_name` values, and conversation. Flagged by research before the schema was written, not discovered after.
**Database:** New `task_activities` table and `task_activity_id` columns applied directly to the live database (same direct-to-Neon pattern used for prior schema changes this session). The existing "FP&A" Main Task (created earlier this session) was renamed to "Financial Planning & Analysis (FP&A)" to match the naming convention used for the other 11 — a display-name-only change, its id and every existing link to it are untouched.
**Self-caught during review:** the Recurring Task form's own Activity auto-fill initially used a simpler version that only filled the title when empty, unlike the Create Task form's version, which also updates an untouched auto-fill when switching to a different Activity. Caught in a self-review pass after the initial implementation and brought both to the same behavior before this shipped.
**Tests:** New test covering an Activity carrying through onto a created task, an inactive Activity being rejected (matching Main Task/Category), the list endpoint's Main Task join, and delete being blocked while in use. Full suite: 82/82 passing. `npm run build` clean.
**Deployment:** Live — verified end-to-end in the browser: the Admin Activities tab renders the real imported data, and the cascading Subtask → Main Task → Activity picker on the Create Task form correctly narrows options at each level and auto-fills the description.
**Cost:** None.

---

**Date:** 2026-09-22
**Change:** Separated master data (the Function/Process/Activity catalog) from actual employee work, per the client's explicit reframing of the Finance task structure. Function (was "Subtask"), Process (was "Main Task"), and Activity are now required — not optional — on every actual task, closing the gap where the catalog enforced its own nesting but a task could still be created pointing at none of it. Activity is now explicitly a TYPE of work (e.g. "Bank Reconciliation"), never auto-copied into a task's description/title anymore — picking one only shapes the placeholder into a concrete example ("Be specific — e.g. Bank Reconciliation for August 2026"), nudging toward the specific instance of work instead of letting the catalog name stand in for it. Added a new Reviewer field (defaults to the employee's manager from the existing reporting hierarchy, editable, shown in History). Delete guards now also block removing a Function/Process while a Process/Activity is still parked under it, not just while a task uses it directly. UI terminology renamed throughout Admin, Team Tasks, and History: "Subtask" → "Function", "Main Task" → "Process" (Activity unchanged) — display text only, no database table/column renames.
**Reason:** Client-directed request: "the system must clearly separate MASTER DATA from ACTUAL EMPLOYEE TASKS," with an explicit rule that Activity = what type of work, Actual Task = what exact work has to be done — plus an explicit instruction not to redesign the whole application, only make the changes required to reach this structure.
**Files:** `server/migrations/001_schema.sql` (new `reviewer_id` column on `commitments`/`recurring_activities` + indexes), `server/src/routes/scrum.js`, `server/src/routes/recurringTasks.js`, `server/src/routes/mainTasks.js`, `server/src/routes/categories.js`, `server/src/routes/taskActivities.js`, `server/src/routes/history.js`, `server/src/lib/recurringOccurrences.js`, `client/src/pages/Admin.jsx`, `client/src/components/TeamTaskList.jsx`, `client/src/pages/History.jsx`, `client/src/components/AuditTimeline.jsx`.
**Database:** `reviewer_id TEXT REFERENCES users(id)` applied directly to the live database on both `commitments` and `recurring_activities`, plus indexes. No other schema change — Function/Process/Activity being "required" is enforced at the application layer (matching how every other required-parent rule in this app already works), not a new `NOT NULL` constraint, since the 21 pre-existing real tasks all predate this feature and have none of these three fields set; tightening validation going forward doesn't touch them.
**Scope check before implementing:** confirmed live against the real database that only one Function ("Finance") exists org-wide and zero of the 21 existing tasks use Function/Process/Activity yet — so making these three required for new tasks carries no risk of breaking in-flight work in some other department that hasn't adopted the catalog.
**Bug found and fixed during live testing:** the Reviewer auto-fill (defaults to the assignee's manager, switching when the assignee changes) initially got the ref-based "is this still untouched" check backwards — the ref tracking the last auto-filled value was overwritten on the line immediately after calling the state updater, so by the time React actually ran the updater (on the next render, not synchronously) the ref already held the NEW default instead of the old one, and the comparison always failed. Caught by testing the real Create Task form in the browser (switching Assign-to from Anudeep to Shreenidhi left Reviewer stuck on Kirubakar instead of updating to Rajeshwari), fixed by snapshotting the ref's old value into a local variable before overwriting it.
**Tests:** New tests: a task cannot be created without each of Function/Process/Activity (one assertion per missing level); Reviewer defaults to the employee's manager but can be explicitly overridden; a Function/Process cannot be deleted while a Process/Activity is parked under it even with zero tasks using either. Existing fixtures that create a task or CSV-import row updated to carry a shared Function/Process/Activity, now that those are required. Full suite: 84/84 passing. `npm run build` clean.
**Deployment:** Live — verified end-to-end in the browser using the client's own example flow: Finance → FP&A → "Annual Operating Plan (AOP) preparation" → created the actual task "Prepare FY27 AOP for BU-01" for Shreenidhi, Reviewer auto-defaulted to Rajeshwari, progressed it Pending → In Progress → Completed, and confirmed the finished record (with Function/Process/Activity/Reviewer all populated) in both Team History's Task Records table and the Audit Log's full change trail.
**Cost:** None.

---

**Date:** 2026-09-22
**Change:** Fixed the Reviewer auto-fill bug described above (flagged there as found during live testing, logged here as its own dated entry since it shipped as a separate commit): switching the "Assign to" employee on the Create Task form left Reviewer stuck on the previous assignee's manager instead of updating to the new assignee's manager.
**Reason:** Caught live in the browser while testing the Function/Process/Activity/Reviewer feature above — switching Assign-to from Anudeep to Shreenidhi left Reviewer showing Kirubakar instead of updating to Rajeshwari.
**Root cause:** A React ref-ordering bug. The component snapshots the last auto-filled Reviewer into a ref so it can tell "did the user manually override this, or is it still on the auto-filled default?" The code was mutating that ref to the NEW default immediately after calling `setState`, but `setState`'s updater function doesn't run synchronously — React runs it on the next render, by which point the ref already held the new value, so the "is this still untouched" comparison always compared the new default against itself and concluded the user had customized it (even when they hadn't).
**Files:** `client/src/components/TeamTaskList.jsx` (Create Task form's Reviewer auto-fill effect).
**Fix:** Snapshot the ref's OLD value into a local variable before mutating the ref, and use that local (not the ref) inside the `setState` updater's comparison.
**Tests:** Covered by manual live browser verification (switching Assign-to and confirming Reviewer follows); this is a UI-effect timing bug, not something the backend test suite exercises.
**Deployment:** Live — verified in the browser.
**Cost:** None.

---

**Date:** 2026-09-22
**Change:** Fixed a display bug: a Function/Process/Activity dropdown could render blank instead of showing the previously-saved value, whenever that saved value belonged to a parent that had since been deactivated (e.g. an Activity under a Process that was later marked inactive). The dropdown's option list only ever included active parents, so a task's own already-saved selection had nowhere to match and silently disappeared from view — the underlying data was untouched, only the on-screen display was wrong.
**Reason:** Found via a targeted investigation prompted by a Vercel deployment the user pointed out ("Fix inline Function/Process selects showing bla...") — confirmed live against real data before fixing.
**Files:** Inline Function/Process `<select>` elements across the affected screens — the option list now always includes the currently-selected value even if it's since been deactivated, instead of filtering to active-only before rendering.
**Tests:** Verified live in the browser against the real deactivated-parent case that originally exposed it.
**Deployment:** Live — verified in the browser; also confirmed via Vercel's own runtime logs (Claude in Chrome) that zero 500 errors occurred in the hour following this deploy.
**Cost:** None.

---

**Date:** 2026-09-22
**Change:** Removed Function from day-to-day use across the entire application. The org has exactly one Function ("Finance") today, so asking every person to pick it on every Process/Activity/Task creation screen was pure friction with no real decision behind it. The Function step is gone from: Admin's Processes, Activities, and Recurring Tasks tabs (both the create forms and the table columns/filters); Team Tasks' Create Task form and filter bar; and History's filter bar and table. Nothing is deleted — Admin's "Functions" tab (full CRUD) stays fully intact for whenever a second Function is actually needed, and all existing data keeps its `category_id`. The backend now auto-resolves Function behind the scenes instead of requiring it: when exactly one active Function exists, it's silently applied; the moment a second active Function ever exists, the app falls back to genuinely requiring an explicit choice again (the same required-field error that existed before, now conditional instead of unconditional) rather than guessing.
**Reason:** User-directed: "remove functions option in the entire application." Clarified via an explicit choice between three options (delete Function entirely and re-normalize the schema; keep it required but hide it from most roles; or stop asking users to pick it while auto-resolving behind the scenes) — the user picked the third: least destructive, keeps the door open for a real second Function later, removes the friction today.
**Files:** `server/src/lib/masterData.js` (new — `resolveDefaultCategoryId()`, the shared auto-resolve helper), `server/src/routes/scrum.js`, `server/src/routes/recurringTasks.js`, `server/src/routes/mainTasks.js` (all three: `category_id`/`category_name` now optional at the API boundary, auto-resolved via the shared helper before falling back to the pre-existing required-field error), `server/src/routes/categories.js` (opportunistic cleanup: 6 leftover "Subtask" error-message strings missed by the earlier Function/Process/Activity terminology rename, fixed to "Function" while touching this same error-message surface), `client/src/pages/Admin.jsx` (Processes/Activities/Recurring Tasks tabs — Function selects, columns, and CSV import headers removed), `client/src/components/TeamTaskList.jsx` (Create Task form and filter bar), `client/src/pages/History.jsx` (filter bar and table), `server/src/routes/history.js` (CSV export header/column removed; underlying SQL join left in place, unused, to minimize backend churn).
**Deliberately unchanged:** `client/src/components/AuditTimeline.jsx`'s `categories`/`category_id` → "Function" label mapping — that interprets historical audit-log entries (what changed, when), a different concern from a live picker, and old audit rows still need a readable label.
**Database:** No schema change. `categories` table, `category_id` foreign keys, and all existing data untouched.
**Tests:** Replaced the now-incorrect assertion "creating a task without Function fails" with two tests: Process/Activity are still required; and a new test proving the auto-resolve contract both ways — omitting Function succeeds and auto-resolves to the single active Function when exactly one exists, but still fails with a clear error once a second Function is (temporarily, for the test) made active. Full suite: 85/85 passing. `npm run build` clean, `npm run lint` clean.
**Deployment:** Live — verified end-to-end in the browser: Admin's Processes/Activities/Recurring Tasks tabs, Team Tasks' Create Task form, and History's filter bar/table all confirmed to have zero Function step; Admin's Functions tab confirmed still fully functional (Finance listed, CRUD available); created a real task with no Function selection and confirmed it auto-resolved to Finance correctly. Zero console errors.
**Cost:** None.
