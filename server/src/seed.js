...
const existingCount = (await db.prepare('SELECT COUNT(*) c FROM users').get()).c;
if (existingCount > 0) {
...
await db.prepare(`INSERT INTO users (id, full_name, email, password_hash, role, job_title, is_super_admin_protected) VALUES (?, ?, ?, ?, ?, ?, 1)`)
  .run(superAdminId, 'Kirubakar B', 'superadmin@dailyscrum.local', hash(DEFAULT_PASSWORD), 'super_admin', 'Super Admin');

await db.prepare(`INSERT INTO users (id, full_name, email, password_hash, role, job_title) VALUES (?, ?, ?, ?, ?, ?)`)
...
await db.prepare('INSERT INTO teams (id, name, leader_user_id) VALUES (?, ?, ?)').run(teamFinanceOps, 'Finance Operations', leader1Id);
await db.prepare('INSERT INTO teams (id, name, leader_user_id) VALUES (?, ?, ?)').run(teamProcurement, 'Procurement', leader2Id);
await db.prepare('INSERT INTO teams (id, name, leader_user_id) VALUES (?, ?, ?)').run(teamCompliance, 'Compliance', leader1Id);

await db.prepare('UPDATE users SET team_id = ? WHERE id = ?').run(teamFinanceOps, leader1Id);
await db.prepare('UPDATE users SET team_id = ? WHERE id = ?').run(teamProcurement, leader2Id);
...
const employeeIds = [];
for (const [full_name, email, team_id, job_title] of employeeNames) {
  const id = uuid();
  await db.prepare(`INSERT INTO users (id, full_name, email, password_hash, role, team_id, job_title) VALUES (?, ?, ?, ?, 'employee', ?, ?)`)
    .run(id, full_name, email, hash(DEFAULT_PASSWORD), team_id, job_title);
  employeeIds.push({ id, full_name, team_id });
}
...
for (const [empIdx, emp] of employeeIds.entries()) {
  const recurringId = uuid();
  await db.prepare('INSERT INTO recurring_activities (id, employee_id, title, frequency) VALUES (?, ?, ?, ?)')
    .run(recurringId, emp.id, pick(RECURRING_TITLES, empIdx), 'Daily');

  for (const [dayIdx, date] of workingDays.entries()) {
    ...
    await db.prepare(`INSERT INTO scrum_sessions (id, employee_id, scrum_date, status, completed_at) VALUES (?, ?, ?, 'completed', ?)`)
      .run(uuid(), emp.id, date, date + 'T09:35:00');
  }
}

// A couple of open actions and one open escalation for realism
await db.prepare(`INSERT INTO actions (id, employee_id, scrum_date, description, owner, due_date, priority, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'open', ?)`)
  .run(uuid(), employeeIds[0].id, today(), 'Get Finance sign-off on pending PR #4521', 'Meena Leader', today(), 'High', today() + 'T09:00:00');

await db.prepare(`INSERT INTO escalations (id, issue, employee_id, escalated_by, escalated_to, required_action, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'open', ?)`)
