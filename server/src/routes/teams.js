...
function assertValidLeader(leaderUserId, res) {
  if (!leaderUserId) return true;
  const leader = db.prepare('SELECT role FROM users WHERE id = ?').get(leaderUserId);
...
router.post('/', requireRole('super_admin', 'admin'), (req, res) => {
  ...
  if (db.prepare('SELECT id FROM teams WHERE lower(name) = lower(?)').get(name.trim())) {
    return res.status(400).json({ error: 'A team with that name already exists.' });
  }
  if (!assertValidLeader(leader_user_id, res)) return;
  const id = uuid();
  db.prepare(`INSERT INTO teams (id, name, leader_user_id, created_by, updated_by) VALUES (?, ?, ?, ?, ?)`)
    .run(id, name.trim(), leader_user_id || null, req.user.id, req.user.id);
  recordAudit({ tableName: 'teams', recordId: id, fieldName: 'created', newValue: name, changedBy: req.user.id, changedByName: req.user.full_name });
...
router.patch('/:id', requireRole('super_admin', 'admin'), (req, res) => {
  ...
  if (name !== undefined && name.trim()) {
    const dupe = db.prepare('SELECT id FROM teams WHERE lower(name) = lower(?) AND id != ?').get(name.trim(), req.params.id);
    if (dupe) return res.status(400).json({ error: 'A team with that name already exists.' });
  }
  if (leader_user_id !== undefined && !assertValidLeader(leader_user_id, res)) return;
  const after = {
  ...
  db.prepare(`UPDATE teams SET name=?, leader_user_id=?, is_active=?, updated_at=datetime('now'), updated_by=? WHERE id=?`)
    .run(after.name, after.leader_user_id, after.is_active, req.user.id, req.params.id);
  auditDiff({ tableName: 'teams', recordId: req.params.id, before, after, changedBy: req.user.id, changedByName: req.user.full_name, reason: req.body?.reason });
...
router.delete('/:id', requireRole('super_admin', 'admin'), (req, res) => {
  ...
  const memberCount = db.prepare('SELECT COUNT(*) c FROM users WHERE team_id = ?').get(req.params.id).c;
  if (memberCount > 0) {
  ...
  recordAudit({ tableName: 'teams', recordId: req.params.id, fieldName: 'deleted', oldValue: team.name, changedBy: req.user.id, changedByName: req.user.full_name, reason: req.body?.reason || req.query?.reason });
