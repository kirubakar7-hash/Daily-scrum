...
router.get('/', asyncHandler(async (req, res) => {
  const rows = await db.prepare('SELECT * FROM task_types ORDER BY is_active DESC, mechanic, name').all();
...
router.patch('/:id', requireRole('super_admin', 'admin'), asyncHandler(async (req, res) => {
  const before = await db.prepare('SELECT * FROM task_types WHERE id = ?').get(req.params.id);
  ...
  await db.prepare(`UPDATE task_types SET name=?, is_active=?, updated_at=datetime('now'), updated_by=? WHERE id=?`)
    .run(after.name, after.is_active, req.user.id, req.params.id);
  await auditDiff({ tableName: 'task_types', recordId: req.params.id, before, after, changedBy: req.user.id, changedByName: req.user.full_name });
  res.json({ task_type: await db.prepare('SELECT * FROM task_types WHERE id = ?').get(req.params.id) });
}));
...
router.delete('/:id', requireRole('super_admin', 'admin'), asyncHandler(async (req, res) => {
  const type = await db.prepare('SELECT * FROM task_types WHERE id = ?').get(req.params.id);
  ...
  const usedCount = (await db.prepare('SELECT COUNT(*) c FROM commitments WHERE task_type_id = ?').get(req.params.id)).c;
  const templateCount = (await db.prepare('SELECT COUNT(*) c FROM recurring_activities WHERE task_type_id = ?').get(req.params.id)).c;
  ...
  await recordAudit({ tableName: 'task_types', recordId: req.params.id, fieldName: 'deleted', oldValue: type.name, changedBy: req.user.id, changedByName: req.user.full_name, reason: req.body?.reason || req.query?.reason });
  await db.prepare('DELETE FROM task_types WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
}));
